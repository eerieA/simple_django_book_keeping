...
class ReviewTestCase(TestCase):
    ...
    def test_reviews(self):
        author = Author.objects.create(last_name="auth_ln", first_name="auth_fn", birth_year=1900)
        ...
        response = self.client.get(book_url)
        self.assertEqual(200, response.status_code)
        self.assertIn(f"{self.users[0].username} says {review_data1['rating']}", str(response.content))
        self.assertIn(f"{self.users[1].username} says {review_data2['rating']}", str(response.content))
